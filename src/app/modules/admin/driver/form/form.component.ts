...
export class FormComponent implements OnInit, FormInterface, OnDestroy{
  ...
  getFactories(): void {
    this.subscription = this._factoryService.loadAll().subscribe((response: any) => {
  ...
  getStates(): void {
    ...
    if (this.operation !== Operation.NEW) {
      this.getCities(this.form.value.state);
    }
  }
  ...
  getCities(idState): void {
    ...
    this.subscription = this._cityService.loadAll(idState).subscribe((response: any) => {
