...
export class FormComponent implements OnInit, FormInterface, OnDestroy {
  ...
  fillForm(): void {
    if (this.operation !== Operation.NEW) {
      this.subscription = this._vehicleTypeService.loadOne(Number(this.id)).subscribe((data: any) => {
        ...
        data.maximumWeightPerAxle = data.maximumWeightPerAxle ? data.maximumWeightPerAxle.toString() : '';
        data.totalGrossPrice = data.totalGrossPrice ? data.totalGrossPrice.toString() : '';
        data.totalGrossPriceTolerance = data.totalGrossPriceTolerance ? data.totalGrossPriceTolerance.toString() : '';
        data.capacity = data.capacity ? data.capacity.toString() : '';
        data.maxLength = data.maxLength ? data.maxLength.toString() : '';
  ...
  onSave(): void {
    if (this._utilsService.formIsValid(this.form)) {
      ...
      let { maximumWeightPerAxle, totalGrossPrice, totalGrossPriceTolerance, capacity, maxLength, quantityOfTheKit } = this.form.value;
      ...
      quantityOfTheKit = parseFloat(quantityOfTheKit);
