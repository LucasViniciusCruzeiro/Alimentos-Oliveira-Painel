...
export class FormComponent implements OnInit, FormInterface, OnDestroy {
  ...
  productType = [
    { id: 'O', description: 'Solução' },
    { id: 'U', description: 'Suspensão' },
    { id: 'A', description: 'Ambos' },
  ];

  registrationType = [
    { id: 'V', description: 'Veículo' },
    { id: 'T', description: 'Tanque' },
  ];
  ...
  fillForm(): void {
    if (this.operation !== Operation.NEW) {
      this.subscription = this._plaqueService.loadOne(Number(this.id)).subscribe((data: any) => {
        ...
        data.tankCapacity = data.tankCapacity ? data.tankCapacity.toString() : '';
