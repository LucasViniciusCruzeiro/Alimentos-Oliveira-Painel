...
export class FormComponent implements OnInit, FormInterface, OnDestroy {
  ...
  setOperation(): void {
    ...
    if (this.id) {
      ...
      this.title = this.operation === Operation.EDIT ? 'Alterando Ingrediente' : 'Visualizando Ingrediente';
    } else {
      this.operation = Operation.NEW;
      this.title = 'Incluindo Ingrediente';
    }
  }

  createForm(): void {
    this.form = new FormGroup({
      idIngredients: new FormControl({ value: null, disabled: true }),
  ...
  fillForm(): void {
    if (this.operation !== Operation.NEW) {
      this.subscription = this._ingredientsService.loadOne(Number(this.id)).subscribe((data: any) => {
  ...
  onSave(): void {
    if (this._utilsService.formIsValid(this.form)) {
      ...
      if (this.operation === Operation.NEW) {
        this._ingredientsService.create(this.form.value).subscribe(data => {
        ...
        this._ingredientsService.update(this.id, this.form.value).subscribe(data => {
          this._swalService.success('Operação realizada com sucesso.').then(res => this.navigate());
        }, (error) => {
          this._swalService.error('Ops', 'Falha ao realizar a operação.');
        });
