...
export class FormComponent implements OnInit, FormInterface, OnDestroy {
  ...
  setOperation(): void {
    ...
    if (this.id) {
      ...
      this.title = this.operation === Operation.EDIT ? 'Alterando Estado' : 'Visualizando Estado';
    } else {
      this.operation = Operation.NEW;
      this.title = 'Incluindo Estado';
    }
  }
  ...
  fillForm(): void {
    if (this.operation !== Operation.NEW) {
      this.subscription = this._stateService.loadOne(Number(this.id)).subscribe((data: any) => {
  ...
  onSave(): void {
    if (this._utilsService.formIsValid(this.form)) {
      ...
      if (this.operation === Operation.NEW) {
        this._stateService.create(this.form.value).subscribe(data => {
        ...
        this._stateService.update(this.id, this.form.value).subscribe(data => {
          this._swalService.success('Operação realizada com sucesso.').then(res => this.navigate());
        }, (error) => {
          this._swalService.error('Ops', 'Falha ao realizar a operação.');
        });
